function [ k_estimate, Y_r_rgtv_cg ] = scgtv_bid( Y_b,k_estimate_size, show_intermediate)
...
%     sigma=0.1;
    ...
    if level >= level_num
        ...
        % k_estimate=fspecial('gaussian', k_size(level), round(k_size(level)/8));
    else
    ...
    for iter=1:3
       ...
       %% kernel estimate
        [ M ] = saturation_correction_mask(Y_r_rgtv_cg);
